%% Code modified from CellReg/demo.m
%% Method overview:
...
rootdir = '/mnt/DATA/Prez/cheeseboard/2019-07/';
animal = 'D-BR';

% Defining the results_directory and creating the figures_directory:
results_directory=[rootdir filesep 'CellReg'];
figures_directory=[results_directory filesep 'Figures'];
if exist(figures_directory,'dir')~=7
    mkdir(figures_directory);
end
figures_visibility='off'; % 'on' or 'off' (in any case figures are saved)

file_names = {};
session_dates = {};
for expname = GetSubdirNames(rootdir)
    expdir = [rootdir filesep expname{1}];
    if startsWith(expname{1}, 'CellReg')
        continue
    end
    for date_str = GetSubdirNames(expdir)
        sfp_file = fullfile(expdir, date_str{1}, 'mv_caimg', animal, 'SFP.mat');
        if isfile(sfp_file)
            file_names{end + 1} = sfp_file;
            session_dates{end + 1} = date_str{1};
        end
    end
end
...
cell_registered_struct.session_dates = session_dates;
...
%% Save cell mapping
cell_mapping = array2table(cell_registered_struct.cell_to_index_map);
cell_mapping.Properties.VariableNames = cellfun(...
    @(x) ['session_' replace(x, '-','_')], ...
    session_dates, 'UniformOutput', 0);
writetable(cell_mapping, fullfile(results_directory, 'cell_mapping.csv'));
